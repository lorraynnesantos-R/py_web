...
class SmartStitch():
    def run(self, ch: Chapter, fn = None) -> Chapter:
        ...
        while continue_index < len(ch.files) - 1:
            ...
            for img in imgs:
                filename=img_handler.save(
                    str(Path.joinpath(Path(tempfile.gettempdir()), 'py_web', Path(ch.files[0]).parent.parent.name, f'{sanitize_folder_name(ch.number)} [stitched]')),
                    img,
                    img_iteration,
                    img_format=conf.img,
                    quality=100,
                )
                files.append(str(Path.joinpath(Path(tempfile.gettempdir()), 'py_web', Path(ch.files[0]).parent.parent.name, f'{sanitize_folder_name(ch.number)} [stitched]', filename)))
                img_iteration += 1
        ...
        shutil.rmtree(Path.joinpath(Path(tempfile.gettempdir()), 'py_web', Path(ch.files[0]).parent.parent.name, f'{sanitize_folder_name(ch.number)} [stitched]'))
